...
class EnhancedStateManager:
    ...
    def _undo_split_operation(self, operation, previous_state, previous_correction):
        ...
        try:
            # 1. 獲取分割前的完整樹狀視圖狀態
            full_tree_state = operation.get('full_tree_state', [])

            if not full_tree_state and previous_state:
                # 如果沒有特殊保存的樹狀視圖狀態，使用前一個狀態
                full_tree_state = previous_state

            if not full_tree_state:
            ...
            # 4. 從保存的狀態重建樹狀視圖
            for item_data in full_tree_state:
                values = item_data.get('values', [])
                position = item_data.get('position', 'end')
    ...
    def undo(self, event=None) -> bool:
        ...
        try:
            ...
            if current_operation and current_operation.get('type') == 'split_srt':
                return self._undo_split_operation(current_operation, previous_state, previous_correction)
            elif current_operation and current_operation.get('type') == 'combine_sentences':
                return self._undo_combine_operation(current_operation, previous_state, previous_correction)
            else:
                # 普通操作的撤銷
                return self._undo_normal_operation(previous_state, previous_correction)
        ...
        try:
            ...
            if hasattr(self, 'alignment_gui'):
                ...
                for item_data in next_state:
                    ...
                    # 插入項目
                    new_id = self.alignment_gui.insert_item('', position, values=tuple(values))

                    # 恢復標籤
                    tags = item_data.get('tags')
                    if tags:
                        self.alignment_gui.tree.item(new_id, tags=tags)

                    # 恢復 use_word_text 狀態
                    if item_data.get('use_word_text', False):
                        self.alignment_gui.use_word_text[new_id] = True
                ...
                # 如果是特殊操作，可能需要額外的處理
                if next_operation and next_operation.get('type') in ['combine_sentences', 'split_srt', 'split_word_text', 'edit_text', 'align_end_times']:
